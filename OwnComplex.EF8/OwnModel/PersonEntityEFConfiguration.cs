...
using static OwnComplex.EF8.OwnModel.EntityFrameworkCommonBuilders;
...
#pragma warning disable SA1305 // Field names should not use Hungarian notation
{
    public class PersonEntityTypeConfiguration : IEntityTypeConfiguration<PersonEntity>
    {
        public void Configure(EntityTypeBuilder<PersonEntity> builder)
        {
            ...
            builder.ComplexProperty(p => p.PersonalDetails!, pdBuilder =>
            {
                pdBuilder.Property(pd => pd.DateOfBirth);
                pdBuilder.ComplexProperty(pd => pd.Gender);
                pdBuilder.ComplexProperty(pd => pd.HomeAddress!, aBuilder =>
                {
                ...
                pdBuilder.ComplexProperty(pd => pd.WorkAddress!, aBuilder =>
                {
                    aBuilder.Property(a => a.AddressLine1).HasMaxLength(100);
                    aBuilder.Property(a => a.AddressLine2).HasMaxLength(100);
            ...
            builder.ComplexProperty(p => p.MedicalDetails!, mBuilder =>
            {
                mBuilder.Property(mi => mi.ProgramingImpairment);
                mBuilder.Property(mi => mi.ProgramingImpairmentLevel).IsRequired(false);
                mBuilder.ComplexProperty(mi => mi.BloodType);
                mBuilder.Property(mi => mi.MedicalConditions).HasJsonConversion();
            ...
            builder.ComplexProperty(p => p.Contact, cBuilder =>
            {
                cBuilder.Property(c => c.EmailAddress).IsRequired();
                cBuilder.ComplexProperty(c => c.PhoneNumbers);
            });
            ...
            builder.ComplexProperty(p => p.RiskProfile);

            builder.ComplexProperty(p => p.Roles);
